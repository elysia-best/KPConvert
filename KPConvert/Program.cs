...
{
    // 打开并定义各种需要的变量
    using var fileStream = File.Open(Psd, FileMode.Open, FileAccess.Read, FileShare.Read);
    // 扫描psd
    ...
    // 解压 kra 文件
    ZipFile.ExtractToDirectory(Kra, tempPath);
    ...
    */
    ...
    {
        Console.WriteLine($"{Psd} 与 {Kra} 图层数不同，处理失败");
        continue;
    }
